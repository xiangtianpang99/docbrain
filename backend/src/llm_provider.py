...
class OpenAIProvider(LLMProvider):
    ...
    def get_crew_llm(self, config: Dict[str, Any]) -> Any:
        ...
        # If using a custom base_url (not OpenAI's), and the model doesn't already have a provider prefix,
        # we often need to tell LiteLLM this is an OpenAI-compatible endpoint by prefixing with 'openai/'.
        # For standard OpenAI (base_url is None or openai.com), 'gpt-4' works fine.
        # But 'deepseek-ai/...' with a custom base_url needs 'openai/' prefix to use the OpenAI client logic.
        if base_url and "api.openai.com" not in base_url and not model.startswith("openai/"):
            model = f"openai/{model}"
